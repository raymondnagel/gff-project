...
public final class StringHelper
{
   ...
   /**
    * Creates a <code>String</code> in which all tabs (<code>'\t'</code>)
    * are converted into the correct number
    * of spaces needed to align the tabbed text by line. However, the
    * tabs will only be aligned if displayed in a monospaced font, such as Lucida Console.
    * 
    * @param     originalText    the text to align.
    * @param     tabSize         the number of blank spaces to replace
    *            a tab with.
    * @return    the aligned text; all tabs are replaced by spaces.
    * @see       #createTabbedMultilineHTML(String[], int)
    */
    public static String alignTabsOfLines(String originalText, int tabSize)
    {
        String[] lines = (originalText + "\r").split("\n");
        int start = 0;
        int[] posts;
        int[] sizes;
        String[] spaces;
        ...
        do
        {            
            posts = findBatchOccurrences(start, lines, "\t");
            sizes = IntegerHelper.subtractArray(posts, start); 
            biggest = IntegerHelper.findHighestPositiveInt(sizes);
            spaces = new String[posts.length];
            for (int s = 0; s < spaces.length; s++)
            {
                spaces[s] = createRepeatedString(" ",(biggest + tabSize) - sizes[s]);
            }
            injectEachListLine(lines, spaces, "\t");
            start += biggest+tabSize;            
        } while (biggest != -1);     
        String result = StringHelper.join(lines, "\n");
